...
namespace Prototype.NetworkLobby
{
    public class LobbyManager : NetworkLobbyManager 
    {
		...
		string last_scene;
		string disconnected_player_id;
		Stack game_order_stack = new Stack ();
		...
		public override void OnServerSceneChanged(string sceneName)
		{
			...
			if (last_scene == lobbyScene || SceneManager.GetActiveScene().name==lobbyScene) return;
		...
		public override void OnServerDisconnect(NetworkConnection conn)
		{
			...
			//if (player_count == 2) GoBackButton ();
        ...
        public override void OnLobbyServerDisconnect(NetworkConnection conn)
        {
            for (int i = 0; i < lobbySlots.Length; ++i)
            {
                ...
                if (p != null)
                {
					...
					p.RpcRemovePlayer (disconnected_player_id);
                }
            }
        ...
        public IEnumerator ServerCountdownCoroutine()
        {
			...
			int [] order = {0,1,2};
			for (int i = 0; i < 3; i++) {
				int c = Random.Range(0, 3-i);
				int t = order[i];
				order[i] = order[i+c];
				order[i+c] = t;
			}
			game_order_stack.Clear ();
			game_order_stack.Push (order[0]);
			game_order_stack.Push (order[1]);
			game_order_stack.Push (order[2]);
			...
			for (int i = 0; i < lobbySlots.Length; ++i)
			{
				if (lobbySlots[i] != null)
				{
					(lobbySlots[i] as LobbyPlayer).RpcClearMatchData();
				}
			}
			// WE COULD ALSO ADD ALL PLAYERS TO MATCH DATA FROM HERE INSTEAD IF ADDING THEM FROM FIRST GAME
